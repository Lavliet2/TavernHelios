...
const EditMenu: React.FC = () => {
  const {
    menuData, dishesData, loading, snackbarMessage, showSnackbar,
    createMenu, removeMenu, addDishToMenu, removeDishFromMenu
  } = useMenu();
            ...
            <List>
              {availableDishes.map((dish) => (
                <ListItem
                  ...
                  onClick={() => {
                    if (selectedMenu) {
                      addDishToMenu(selectedMenu, dish.id);
                    }
                  }}
      ...
      <Modal open={isCreateMenuModalOpen} onClose={() => setIsCreateMenuModalOpen(false)}>
        ...
        >
          ...
          <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
            <Button
              variant="contained"
              onClick={() => createMenu(newMenuName)}
              color="primary"
              sx={{ mr: 2 }}
              disabled={!newMenuName.trim()} // Защита от пустого ввода
            >
              Создать
            </Button>
