...
const DishAddModal: React.FC<DishAddModalProps> = ({ dish, open, onClose, onSave, onChange, onImageUpload }) => {
    ...
    <Modal open={open} onClose={onClose}>
      <Box sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", bgcolor: "white", p: 4, borderRadius: 2, width: 400 }}>
        <Typography variant="h6" sx={{ mb: 2 }}>
          {t("editDishes.addDish")}
        </Typography>
        <TextField label={ t("common.name") } fullWidth required value={dish.name} onChange={(e) => onChange("name", e.target.value)} sx={{ mb: 2 }} />
        ...
        <FormControl fullWidth sx={{ mb: 2 }} required>
          <InputLabel>{ t("editDishes.dishType") }</InputLabel>
